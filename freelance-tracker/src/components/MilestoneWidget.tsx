    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-white/70 text-[11px] font-semibold uppercase tracking-wide">{t('milestone.label')}</p>
            <p className="text-white text-[15px] font-bold mt-1">{t('milestone.noActive')}</p>
          </div>
          <Sparkles size={18} className="text-white/70 shrink-0" />
        </div>
        <p className="text-white/70 text-[11px] mt-4">
          {t('milestone.setEndHint')}
        </p>
      </div>
    )
  }
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 relative">
        <div className="min-w-0">
          <p className="text-white/70 text-[10px] font-semibold uppercase tracking-wide">{t('milestone.label')}</p>
        ...
        <div className="shrink-0 flex flex-col items-end gap-1">
          ...
          <span className="text-white text-[11px] font-bold bg-white/15 px-2 py-0.5 rounded-full whitespace-nowrap">
            {t('milestone.daysLeft', { n: daysRemaining })}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4 relative">
        <div>
          <p className="text-white/60 text-[9px] font-semibold uppercase tracking-wider">{t('milestone.hours')}</p>
        ...
        <div>
          <p className="text-white/60 text-[9px] font-semibold uppercase tracking-wider">{t('milestone.tasks')}</p>
      ...
      <div className="relative mt-4">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-white/80 text-[11px] font-medium">{t('milestone.progress')}</span>
      ...
      {nextTask && (
        <div className="relative mt-3 pt-3 border-t border-white/15">
          <p className="text-white/60 text-[9px] font-semibold uppercase tracking-wider mb-1">{t('milestone.upNext')}</p>
      ...
      <div className="relative mt-auto pt-4">
        <div className="flex items-center gap-1.5 text-white/80 text-[11px]">
          <Calendar size={11} />
          <span>{formatRange(start, end, locale)}</span>
        </div>
      </div>
    </button>
  )
