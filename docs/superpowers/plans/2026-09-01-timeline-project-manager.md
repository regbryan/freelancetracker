...
- [x] **Step 2 — Gantt.** `milestones` prop, milestone rows with chevron, `done/total` and a progress-filled bar spanning its own dates or its tasks' extent, tasks nested one level and collapsed by default, "Unassigned" group only when the project has milestones, drag/resize when editable, Overview diamonds at milestone end dates, hide-done still hides tasks only.
  - Chevron and name are one `aria-expanded` button (two buttons with the same name read as a duplicate); expansion is controlled by the page with a local `useState` fallback, and Overview is detected as `tasks.length === 0 && milestones.length > 0`. Only a milestone with both its own dates is draggable — one that follows its tasks gets no handles and a "Dates follow its tasks" title.
  - Visual review at 1280 in Week zoom, collapsed and expanded, fixed two things the spec left ambiguous: the bar label flips white/navy on *pixels* of fill under it, not on the done ratio (navy on accent green is 2.3:1), and the Overview diamond needs a hairline `border-surface` outline or its half over the same-navy project bar disappears. `tsc` clean, lint 0 errors (20 warnings, unchanged), 213 tests passing (was 199).
- [x] **Step 3 — page and forms.** "+ Milestone" button and dialog (name, start, end, delete) on `/timeline`, expansion state per project in `localStorage['timeline.expanded.<projectId>']`, `TaskForm` milestone select passed from the Timeline page, i18n keys in both dictionaries.
  - Closed two seams step 2 left: the Gantt takes an explicit `mode` prop instead of inferring Overview from an empty task list (a project whose tasks are all hidden is still a project view), and a `hideDone` prop, so the page hands over every task and the Gantt counts them all for `done/total` while rendering only the open ones — a finished milestone keeps its bar at `n/n`. Milestone bars are 18px like project bars; 14px under 16px task bars inverted the hierarchy. The track range now includes each milestone's *drawn* span so a milestone borrowing the extent of hidden done tasks still fits.
  - `MilestoneForm` follows `TaskForm`'s dialog vocabulary; date order is checked inline (equal dates are a one-day milestone, not an error) and Delete asks a native `confirm` that says the tasks survive. Page errors reuse the existing banner and re-throw so the dialog stays open. New milestones take `max(sort_order) + 1`. Expansion is seeded per project by adjusting state during render, not in an effect, so a project's first paint is already its remembered shape.
